...
RSpec.describe 'occupational sessions', js: true, type: :feature do
  ...
  scenario 'A user can capture a new occupational session' do
    ...
    expect(page.evaluate_script('angular.element(document.getElementById("sessions_new_session_controller")).scope().session.group')).to eq 'occupational'
    ...
    expect(page.evaluate_script('angular.element(document.body).scope().sessions[0].group')).to eq 'occupational'
  end
end
